...
const MOCK_SESSIONS = [
  { id: 1, prompt: "Explain how DNS works",        intent: "process",    frameCount: 3, time: "2 hours ago", accent: "#a5d8ff" },
  { id: 2, prompt: "Newton's second law F = ma",   intent: "math",       frameCount: 4, time: "Yesterday",   accent: "#b2f2bb" },
  { id: 3, prompt: "TCP vs UDP comparison",        intent: "comparison", frameCount: 2, time: "2 days ago",  accent: "#ffec99" },
]
...
const SessionCard = ({ session, isSelected, onClick }) => {
  ...
  const meta   = INTENT_META[session.intent] || { label: session.intent, bg: '#f1f5f9', text: '#64748b' }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', gap: 0.5, mb: 1 }}>
        {Array.from({ length: Math.min(session.frameCount, 3) }).map((_, i) => (
          <Box
            key={i}
            sx={{
              flex: 1,
              height: 26,
              borderRadius: '5px',
              backgroundColor: isDark ? session.accent + '55' : session.accent,
              opacity: 0.5 + i * 0.2,
            }}
          />
        ))}
...
const EmptyView = ({ onSuggestionClick }) => {
  ...
  return (
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2.5 }}>
      ...
      <Box sx={{ textAlign: 'center' }}>
        <Typography sx={{ fontWeight: 700, fontSize: 17, color: theme.palette.text.primary, mb: 0.75 }}>
...
const ResultView = ({ result }) => {
  const theme  = useTheme()
  const isDark = theme.palette.mode === 'dark'
  const meta   = INTENT_META[result.intent_type] || { label: result.intent_type, bg: '#f1f5f9', text: '#64748b' }

  return (
    <Box sx={{ p: 3, overflowY: 'auto', flex: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
        <Chip
          label={`${meta.label} · ${result.frame_count} frames`}
          size="small"
        ...
        {(result.captions || []).map((caption, i) => (
          <Box
            ...
            sx={{
              ...
              '&:hover': {
                boxShadow: isDark ? `0 0 0 1px ${theme.palette.divider}` : '0 4px 12px rgba(0,0,0,0.08)',
          ...
          >
            ...
            <Box
              sx={{
                height: 148,
                backgroundColor: isDark ? meta.bg + '22' : meta.bg,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                position: 'relative',
            ...
            >
              {result.images?.[i] ? (
                <img
                  src={`file://${result.images[i]}`}
                  alt={caption}
                  style={{ width: '100%', height: '100%', objectFit: 'contain' }}
                />
              ) : (
                <Box sx={{ textAlign: 'center' }}>
                  <MovieOutlinedIcon sx={{ fontSize: 28, color: meta.text, opacity: 0.3 }} />
                  <Typography sx={{ fontSize: 11, color: meta.text, opacity: 0.35, mt: 0.5 }}>
                    Frame {i + 1}
                  </Typography>
                </Box>
              )}
              ...
              >
                <Typography sx={{ fontSize: 10, color: '#fff', fontWeight: 600 }}>
                  {i + 1}/{result.frame_count}
                </Typography>
              </Box>
            </Box>

            <Box sx={{ px: 1.5, py: 1.25 }}>
              <Typography sx={{ fontSize: 12.5, fontWeight: 600, color: theme.palette.text.primary }}>
                {caption}
              </Typography>
            </Box>
          </Box>
...
export default function Studio() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const session = { id: Date.now(), prompt: prompt.trim(), intent: data.intent_type, frameCount: data.frame_count, time: 'Just now', accent: '#c7d2fe', data }
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.default' }}>
      ...
      >
        ...
        <Tooltip title="New session">
          <IconButton
            size="small"
            onClick={() => { setResult(null); setSelected(null); setPrompt(''); inputRef.current?.focus() }}
            sx={{
              color: theme.palette.primary.main,
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map((s) => (
                <SessionCard
                  ...
                  onClick={() => { setSelected(s.id); if (s.data) setResult(s.data) }}
                />
              ))
            )}
          </Box>
