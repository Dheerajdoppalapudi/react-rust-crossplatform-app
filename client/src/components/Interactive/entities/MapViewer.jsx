...
// Fix Leaflet broken default icon paths in bundlers
        ...
        >
          ...
          {polygons.map((pg, i) => (
            <Polygon
              key={i}
              positions={pg.points}
              color={pg.strokeColor ?? '#4B72FF'}
              fillColor={pg.fillColor ?? '#4B72FF'}
              fillOpacity={pg.opacity ?? 0.3}
              weight={pg.weight ?? 2}
            >
              {(pg.label || pg.tooltip) && (
                <Popup>
          ...
          {circles.map((c, i) => (
            <Circle
              ...
              fillColor={c.fillColor ?? c.color ?? '#f59e0b'}
