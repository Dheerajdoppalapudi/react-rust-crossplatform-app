...
export default function SessionNode({ data }) {
  ...
  const intentLabel = formatIntentType(turn.intent_type)
  const isReady     = !isText && turn.videoPhase === 'ready'

  const { videoUrl, getFrameUrl } = useMediaUrl(turn.id)
  const frameUrl = getFrameUrl(0)

  // Reset broken-image flag when the session changes (e.g. after retry)
  useEffect(() => { setImgError(false) }, [turn.id])

  // Read video duration from metadata for the ready badge
  useEffect(() => {
    ...
    <>
      ...
      >
        <Tooltip title="Ask a follow-up" placement="right" arrow>
          <Box
            ...
            sx={{
              ...
              '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.13)' : 'rgba(0,0,0,0.09)' },
            }}
          >
        ...
        <Tooltip
          ...
          placement="top" arrow enterDelay={400}
        >
          <Box sx={{
            width: NODE_W, height: NODE_H,
            borderRadius: '10px',
            ...
            >
              {isText ? (
                ...
                <img
                  src={frameUrl} alt="thumbnail" onError={() => setImgError(true)} draggable={false}
                  style={{
              ...
              {!isText && thumbHover && turn.id && !imgError && frameUrl && (
