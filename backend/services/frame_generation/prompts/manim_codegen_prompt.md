...
self.play(dot.animate.move_to(axes.c2p(new_val, fn(new_val))), run_time=0.6)
```
...
class GeneratedScene(Scene):
    def construct(self):
        ...
        self.play(Create(dot), FadeIn(start_lbl), run_time=0.4)
        self.wait(0.3)
        ...
        for i in range(len(steps) - 1):
            ...
            self.play(Create(arrow), FadeIn(grad_lbl), run_time=0.3)
            self.play(FadeOut(arrow), FadeOut(grad_lbl), run_time=0.2)
            if i == 0:
                self.play(FadeOut(start_lbl), run_time=0.1)
            self.play(dot.animate.move_to(axes.c2p(x_nxt, x_nxt**2)), run_time=0.5)
            self.wait(0.2)
        ...
        self.play(FadeIn(star), Write(min_lbl), run_time=0.5)
        self.wait(0.5)
