...
async def _generate_one_manim_frame(
    ...
    logger.warning(
        "manim_frame_render_failed_retrying",
        frame=frame_index, category=error_category, bad=bad_name or None,
    )
    ...
    if error_category == "constructor_kwargs" and bad_name:
        ...
        if stripped != code:
            logger.info("manim_frame_kwarg_strip", frame=frame_index, bad=bad_name)
