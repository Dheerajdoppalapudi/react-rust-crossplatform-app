...
async def _generate_one_frame(
    ...
    constraints into prompt_template.md, then calls the LLM via call_llm().

    The style note forces the model to use the same colors and roughness
    as every other frame, keeping the combined canvas visually consistent
    even though each frame is generated in a separate independent call.
    """
    style_note = (
...
async def generate_all_frames(
    ...
    """
    tasks = [
        _generate_one_frame(frame, plan.shared_style, prompt_template)
