...
def export_frames(
    ...
    for i, (src_path, caption) in enumerate(zip(png_paths, captions)):
        ...
        if src_path and src_path.lower().endswith(".mp4") and os.path.exists(src_path):
            logger.debug("frame_export_passthrough_mp4", frame=i, path=src_path)
            output_paths.append(src_path)
            continue

        if src_path and os.path.exists(src_path):
            logger.debug("frame_export_load_png", frame=i, path=src_path)
            img = Image.open(src_path)
        else:
            if src_path:
                logger.warning("frame_export_png_missing", frame=i, path=src_path)
            else:
                logger.debug("frame_export_no_path_placeholder", frame=i)
